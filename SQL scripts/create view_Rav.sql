CREATE OR REPLACE VIEW view_Rav AS
...
SELECT Rav.ravID, Rav.ravName, Student.stdID, Student.name, Student.address, Student.phone, bmhName, volGroupOrg, volGroupPhone, booksLearning
FROM Student INNER JOIN Student_BmhName ON Student.stdID = Student_BmhName.stdID
     ...
     INNER JOIN Rav ON Student.ravID = Rav.ravID
ORDER BY Rav.ravName
